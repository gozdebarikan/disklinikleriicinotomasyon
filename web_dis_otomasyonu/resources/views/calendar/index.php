...
<div class="card">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; flex-wrap:wrap; gap:16px;">
        <a href="<?= url('/') ?>?week=<?= $weekOffset - 1 ?>&doc=<?= $_GET['doc']??1 ?>" class="btn btn-secondary btn-sm">
                ...
                <tbody>
                    <?php foreach($myAppointments as $index => $a): ?>
                        <tr style="border-bottom:1px solid var(--gray-100); transition:var(--transition-fast);" 
                            ...
                            <td style="padding:16px;">
                                <?php 
                                    // Fix: Check for both 'cancelled' (English) and 'iptal' (Turkish DB value)
                                    $isActive = ($a['status'] !== 'cancelled' && $a['status'] !== 'iptal');
                            ...
                            <td style="padding:16px; text-align:right;">
                                <?php if($a['status'] !== 'cancelled' && $a['status'] !== 'iptal'): ?>
...
function updateCalendar() {
    ...
    window.location.href = '<?= url("/") ?>?doc=' + docId;
}
...
function openSlotModal(date, display) {
    ...
    // Fetch slots
    const docId = document.getElementById('doctor-select').value;
    fetch(`<?= url('/api/slots') ?>?date=${date}&doctor_id=${docId}`, {
