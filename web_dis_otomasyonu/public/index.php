...
// GLOBAL URL HELPER
// SCRIPT_NAME: /DentalApp/dental-app4/public/index.php
// dirname: /DentalApp/dental-app4/public
$GLOBALS['basePath'] = dirname($_SERVER['SCRIPT_NAME']);
// Windows ters slash düzeltmesi
$GLOBALS['basePath'] = str_replace('\\', '/', $GLOBALS['basePath']); 
// Eğer root ise (/) sağdaki slash'ı kaldır, ama dirname genelde slashsız döner (root hariç)
if ($GLOBALS['basePath'] === '/' || $GLOBALS['basePath'] === '\\') $GLOBALS['basePath'] = '';
...
$projectRoot = dirname($GLOBALS['basePath']);

// URI'den proje root'unu çıkar
if (strpos($requestUri, $GLOBALS['basePath']) === 0) {
    if ($GLOBALS['basePath'] === '') {
        $uri = $requestUri;
    } else {
        $uri = substr($requestUri, strlen($GLOBALS['basePath']));
    }
} elseif (strpos($requestUri, $projectRoot) === 0) { // Root URL erişimi (public olmadan)
    if ($projectRoot === '' || $projectRoot === '/' || $projectRoot === '\\') {
         $uri = $requestUri;
    } else {
         $uri = substr($requestUri, strlen($projectRoot));
    }
} elseif (strpos($requestUri, '/dental-app4') === 0) { // Fallback
    $uri = str_ireplace('/dental-app4', '', $requestUri);
} else {
    $uri = $requestUri;
}


// Public klasörü varsa onu da temizle (bazen rewrite çalışmazsa public explicit gelebilir)
