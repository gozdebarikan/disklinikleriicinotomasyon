...
class AppointmentRepository extends BaseRepository {
    ...
    public function cancelAppointment(int|string $id, int|string $patientId): bool {
        ...
        // Doğrudan SQL sorgusu ile güncelleme yap ve rowCount kontrolü sağla
        $stmt = $this->db->prepare("UPDATE {$this->table} SET durum = 'iptal' WHERE id = ?");
        $result = $stmt->execute([$id]);
        
        if ($result && $stmt->rowCount() === 0) {
            // Hata yok ama güncellenen kayıt yok
            return false;
        }
