...
class AppointmentService {
    ...
    public function bookAppointment($patientId, array $data) {
        try {
            ...
            if ($appointmentDateTime < time()) {
                throw new \Exception('Geçmiş bir tarih/saat için randevu alınamaz.');
            }
            
            // Aktif randevu kontrolü - hasta zaten bekleyen randevusu varsa
            if ($this->appointmentRepo->hasActiveAppointment($patientId)) {
                throw new \Exception('Zaten aktif bir randevunuz bulunmaktadır. Yeni randevu almak için mevcut randevunuzu iptal edin veya tamamlanmasını bekleyin.');
            }
    ...
    public function getTakenSlots(int|string $doctorId, string $date): array {
        return $this->appointmentRepo->getTakenSlots($doctorId, $date);
    }
}
